...
class ExperimentDatabase:
    ...
    def insert_experiment(
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                ...
                (name, experiment_type, parameters, results, timestamp, git_commit, hostname)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    name,
    ...
    def delete_experiment(self, experiment_id: int) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "DELETE FROM instances WHERE experiment_id = ?",
                (experiment_id,)
            )
            cursor = conn.execute(
    ...
    def list_experiments(self) -> List[Dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute(
                "SELECT id, name, experiment_type, timestamp FROM experiments ORDER BY timestamp DESC"
            )
            rows = cursor.fetchall()
