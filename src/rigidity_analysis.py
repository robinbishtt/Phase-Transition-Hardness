...
def compute_rigidity_threshold_indicator(
    ...
    """Return True if the estimated frozen fraction exceeds the threshold.

    Samples n_samples WalkSAT solutions to estimate the mean frozen fraction
    of the instance.  Returns a Python bool (not a NumPy scalar) so that the
    result can be used in standard boolean contexts and isinstance checks.
    """
    result = estimate_cluster_rigidity(instance, n_samples=50)
    return bool(result["mean"] > threshold)
