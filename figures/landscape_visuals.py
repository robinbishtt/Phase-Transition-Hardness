...
def _phase_diagram(output_dir, fmt, dpi):
    ...
    ax.plot(alphas, entropy    / np.log(2), color="
            label=r"Entropy density $s(\alpha)/\log 2$")
    ax.plot(alphas, complexity * 2,          color="
            label=r"Cluster complexity $2\Sigma(\alpha)$")
    ax.plot(alphas, frozen,                  color="
            label=r"Frozen fraction $f_{\mathrm{frz}}(\alpha)$")
    ax.plot(alphas, b_curve / 0.015,         color="
...
def _rigidity_complexity(output_dir, fmt, dpi):
    ...
    ax1.plot(alphas, frozen, color="
             label=r"Frozen fraction $f_{\mathrm{frz}}$")
    ax2.plot(alphas, sigma,  color="
    ...
    ax1.set_ylabel(r"Frozen fraction", color="
    ax2.set_ylabel(r"Cluster complexity $\Sigma$", color="
...
def _barrier_height_scaling(output_dir, fmt, dpi):
    ...
    b_vals = [0.005, 0.012, 0.008]
    colors = ["

    fig, ax = plt.subplots(figsize=(6, 4.2))

    for b, color in zip(b_vals, colors):
        barriers = b * ns
