...
from src.energy_model import ALPHA_S, ALPHA_STAR
...
def generate_scaling_collapse(
    ...
    ax = axes[1]
    # Hardness FSS variable centred at α*≈4.20 (manuscript Eq. 16),
    # NOT at α_s=4.267.  The P_sat collapse (left panel) centres at α_s;
    # the hardness-density collapse (right panel) centres at α*.
    alpha_collapse = ALPHA_STAR  # 4.20, peak-hardness density
    for i, n in enumerate(ns):
        color  = COLORS[i % len(COLORS)]
        x_coll = (alphas - alpha_collapse) * n ** (1.0 / nu)
