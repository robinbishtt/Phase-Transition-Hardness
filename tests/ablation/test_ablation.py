...
class TestEnergyModelAblation(unittest.TestCase):
    ...
    def test_entropy_without_rs_correction(self):
        """Test that the RS entropy approximation satisfies 0 ≤ s_RS ≤ log(2).

        The code's K=3 RS formula is a power-law calibration fitted to cavity
        numerics; it does not guarantee s_RS ≤ s_annealed because the annealed
        entropy for K=3 becomes negative above α ≈ 2.41.  The correct bounds
        for this approximation are 0 ≤ s_RS ≤ log(2).
        """
        ...
        for alpha in [0.5, 1.0, 1.5, 2.0, 3.0, 4.0]:
            ...
            self.assertGreaterEqual(rs, 0.0,
                msg=f"s_RS({alpha}) = {rs:.6f} < 0")
            self.assertLessEqual(rs, np.log(2) + 1e-10,
                msg=f"s_RS({alpha}) = {rs:.6f} > log(2)")
