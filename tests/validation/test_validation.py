...
class TestThresholdValues(unittest.TestCase):
    ...
    def test_threshold_ordering(self):
        """For K=3: α_d = α_r and α_c = α_s, so ordering uses ≤."""
...
class TestBarrierProperties(unittest.TestCase):
    ...
    def test_barrier_peak_value(self):
        """Maximum barrier density should be approximately 0.021 (manuscript Table 4, H∞ = 0.0210)."""
        max_barrier = barrier_density(4.2)
        self.assertAlmostEqual(max_barrier, 0.021, delta=0.003)
...
class TestComplexityProperties(unittest.TestCase):
    ...
    def test_complexity_onset_at_alpha_d(self):
        """Complexity rises from zero just above α_d; onset value is near-zero."""
        c_onset = cluster_complexity(ALPHA_D + 0.001)
        self.assertGreater(c_onset, 0.0)
        self.assertLess(c_onset, 0.01)

    def test_complexity_maximum_at_alpha_d(self):
        """Complexity peaks near α≈4.05 (not at α_d); Σ_max ≈ 0.047."""
...
class TestExponentialScalingConjecture(unittest.TestCase):
    ...
    def test_exponential_scaling_fit_quality(self):
        ...
        gamma = 0.01
        log_means = gamma * ns + 2.0 + rng.randn(5) * 0.01   # small noise
