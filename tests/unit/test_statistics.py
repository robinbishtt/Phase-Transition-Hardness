...
class TestFitLognormal(unittest.TestCase):
    ...
    def test_perfect_lognormal_data(self):
        """Should fit correctly to lognormal data (deterministic with fixed seed)."""
        rng = np.random.RandomState(42)
        mu_true, sigma_true = 0.0, 1.0
        data = rng.lognormal(mu_true, sigma_true, size=1000)
        result = fit_lognormal(data)
        self.assertAlmostEqual(result["mu"], mu_true, delta=0.1)
        self.assertAlmostEqual(result["sigma"], sigma_true, delta=0.1)
